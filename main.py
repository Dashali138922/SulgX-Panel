...
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&family=Orbitron:wght@700;900&family=Vazirmatn:wght@400;600;700;800&display=swap" rel="stylesheet">
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
<style>
*{margin:0;padding:0;box-sizing:border-box}
:root{
  --primary:#7c3aed; --primary-light:#a78bfa; --primary-dim:rgba(124,58,237,0.15);
  --bg:#0a0a0f; --bg2:#12121f; --bg3:#1a1a2e;
  --surface:rgba(20,20,35,0.85); --surface2:rgba(30,30,50,0.9); --surface3:rgba(40,40,60,0.8);
  --border:rgba(124,58,237,0.12); --border2:rgba(124,58,237,0.25);
  --text:#e8e8f0; --text2:#a8a8b8; --text3:#787888;
  --green:#34d399; --red:#f87171; --yellow:#fbbf24; --blue:#60a5fa;
  --header-h:64px; --footer-h:48px;
  --radius:20px; --radius-sm:12px;
  --shadow:0 8px 32px rgba(0,0,0,0.4);
}
body.light-mode {
  --bg:#f4f4f9; --bg2:#ffffff; --bg3:#e8e8f0;
  --surface:rgba(255,255,255,0.85); --surface2:rgba(255,255,255,0.95); --surface3:rgba(240,240,248,0.9);
  --border:rgba(0,0,0,0.08); --border2:rgba(124,58,237,0.2);
  --text:#1a1a2e; --text2:#4a4a5e; --text3:#8888a0;
  --shadow:0 8px 32px rgba(0,0,0,0.08);
}
body.blue-mode {
  --primary:#3b82f6; --primary-light:#93bbfc; --primary-dim:rgba(59,130,246,0.15);
  --bg:#0f172a; --bg2:#1e293b; --bg3:#1e293b;
  --surface:rgba(30,41,59,0.85); --surface2:rgba(30,41,59,0.95); --surface3:rgba(51,65,85,0.8);
  --border:rgba(59,130,246,0.15); --border2:rgba(59,130,246,0.3);
  --text:#e2e8f0; --text2:#94a3b8; --text3:#64748b;
  --shadow:0 8px 32px rgba(0,0,0,0.3);
}
html,body{height:100%;overflow-x:hidden}
body{font-family:'Inter','Vazirmatn',sans-serif;color:var(--text);display:flex;flex-direction:column;background:var(--bg);transition:all 0.35s ease;scroll-behavior:smooth}
body[dir="rtl"]{direction:rtl;text-align:right}
::-webkit-scrollbar{width:6px;height:6px}
::-webkit-scrollbar-track{background:var(--bg2)}
::-webkit-scrollbar-thumb{background:var(--primary);border-radius:10px}
a{text-decoration:none;color:inherit}
.glass{background:var(--surface);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid var(--border);border-radius:var(--radius)}
.glass-sm{background:var(--surface2);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid var(--border);border-radius:var(--radius-sm)}
.shadow-glow{box-shadow:0 0 40px var(--primary-dim),var(--shadow)}
.header{height:var(--header-h);background:var(--surface);border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 20px;position:sticky;top:0;z-index:100;backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px)}
.header-inner{display:flex;align-items:center;justify-content:space-between;width:100%;max-width:1440px;margin:0 auto}
.logo{font-family:'Orbitron',sans-serif;font-size:1.5rem;font-weight:900;background:linear-gradient(135deg,var(--primary),var(--primary-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:1px}
.logo span{font-size:0.65rem;font-family:'Inter';-webkit-text-fill-color:var(--text3);background:none;margin-left:6px;font-weight:600}
.header-nav{display:flex;align-items:center;gap:4px}
.nav-link{padding:8px 16px;border-radius:var(--radius-sm);color:var(--text3);font-size:0.85rem;font-weight:600;transition:all 0.25s;border:none;background:none;cursor:pointer;font-family:inherit;position:relative}
.nav-link::after{content:'';position:absolute;bottom:2px;left:50%;width:0;height:2px;background:var(--primary);border-radius:4px;transition:all 0.3s;transform:translateX(-50%)}
...
.btn{font-family:inherit;font-size:0.85rem;font-weight:700;border-radius:var(--radius-sm);padding:8px 18px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;border:none;transition:all 0.25s}
.btn-primary{background:linear-gradient(135deg,var(--primary),var(--primary-light));color:#fff;box-shadow:0 4px 16px var(--primary-dim)}
.btn-primary:hover{transform:translateY(-2px);box-shadow:0 8px 28px var(--primary-dim)}
.btn-outline{background:var(--surface3);color:var(--text);border:1px solid var(--border)}
.btn-outline:hover{background:var(--primary-dim);border-color:var(--primary);color:var(--primary)}
...
.btn-icon{background:transparent;border:1px solid var(--border);color:var(--text3);border-radius:var(--radius-sm);padding:8px 10px;cursor:pointer;transition:all 0.25s;font-size:1.1rem}
.btn-icon:hover{color:var(--primary);border-color:var(--primary);background:var(--primary-dim)}
.lang-switch{display:flex;gap:3px;background:var(--surface3);border-radius:var(--radius-sm);padding:3px}
.lang-btn{padding:5px 12px;border:none;background:transparent;color:var(--text3);font-size:0.75rem;font-weight:700;border-radius:8px;cursor:pointer;font-family:inherit;transition:all 0.2s}
.lang-btn.active{background:var(--primary);color:#fff;box-shadow:0 2px 12px var(--primary-dim)}
.hamburger{display:none;background:transparent;border:1px solid var(--border);color:var(--text3);font-size:1.6rem;cursor:pointer;padding:4px 12px;border-radius:var(--radius-sm)}
.main{flex:1;padding:20px 24px;max-width:1440px;margin:0 auto;width:100%;min-height:calc(100vh - var(--header-h) - var(--footer-h))}
...
.page-title{font-size:1.6rem;font-weight:800;background:linear-gradient(135deg,var(--primary),var(--primary-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.page-sub{font-size:0.9rem;color:var(--text3);margin-top:2px}
.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px;margin-bottom:20px}
.stat-card{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:18px 20px;transition:all 0.3s;position:relative;overflow:hidden}
.stat-card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--primary),var(--primary-light));opacity:0;transition:opacity 0.3s}
...
.stat-label{font-size:0.7rem;color:var(--text3);font-weight:700;text-transform:uppercase;letter-spacing:0.06em}
.stat-val{font-size:1.5rem;font-weight:800;color:var(--text);margin-top:2px}
.stat-unit{font-size:0.8rem;font-weight:400;color:var(--text3);margin-left:4px}
.card{background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:20px;margin-bottom:16px;transition:all 0.3s}
...
.tbl th{text-align:left;padding:12px 14px;background:var(--surface3);color:var(--text2);font-weight:700;font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;border-bottom:2px solid var(--border)}
...
.tbl input[type=checkbox]{width:16px;height:16px;accent-color:var(--primary);cursor:pointer}
.tag{display:inline-flex;align-items:center;padding:3px 10px;border-radius:20px;font-size:0.7rem;font-weight:700;text-transform:uppercase;gap:4px}
.tag-vless{background:var(--primary-dim);color:var(--primary);border:1px solid var(--border2)}
.tag-on{background:rgba(52,211,153,0.12);color:var(--green);border:1px solid rgba(52,211,153,0.2)}
.tag-off{background:rgba(248,113,113,0.12);color:var(--red);border:1px solid rgba(248,113,113,0.2)}
...
.pill-fill{height:100%;border-radius:4px;transition:width 0.6s ease}
.pill-lim{color:var(--text3);font-size:0.75rem;white-space:nowrap}
.toggle{width:42px;height:24px;border-radius:12px;background:var(--surface3);position:relative;cursor:pointer;transition:all 0.3s;border:2px solid var(--border);flex-shrink:0}
.toggle::after{content:'';position:absolute;width:16px;height:16px;border-radius:50%;background:var(--text3);top:2px;left:3px;transition:all 0.3s}
...
.act-btn{font-family:inherit;font-size:0.7rem;font-weight:700;padding:4px 8px;border-radius:6px;cursor:pointer;border:1px solid;transition:all 0.2s;background:transparent;display:inline-flex;align-items:center;gap:3px}
.act-btn:hover{transform:scale(1.05)}
.act-copy{color:var(--primary);border-color:var(--border)}
.act-copy:hover{background:var(--primary-dim)}
.act-sub{color:var(--green);border-color:rgba(52,211,153,0.2)}
.act-sub:hover{background:rgba(52,211,153,0.1)}
.act-qr{color:var(--blue);border-color:rgba(96,165,250,0.2)}
.act-qr:hover{background:rgba(96,165,250,0.1)}
.act-edit{color:var(--yellow);border-color:rgba(251,191,36,0.2)}
.act-edit:hover{background:rgba(251,191,36,0.1)}
.act-del{color:var(--red);border-color:rgba(248,113,113,0.2)}
.act-del:hover{background:rgba(248,113,113,0.1)}
.fi,.fs{padding:10px 14px;border-radius:var(--radius-sm);border:1px solid var(--border);font-family:inherit;font-size:0.9rem;outline:none;color:var(--text);background:var(--surface);transition:all 0.25s;width:100%}
.fi:focus,.fs:focus{border-color:var(--primary);box-shadow:0 0 0 3px var(--primary-dim)}
.fl{font-size:0.75rem;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:0.04em;margin-bottom:4px}
.fg{display:flex;flex-direction:column;gap:4px;margin-bottom:14px}
.chip-group{display:flex;flex-wrap:wrap;gap:6px}
.chip{padding:6px 16px;border-radius:20px;font-size:0.8rem;font-weight:700;color:var(--text3);cursor:pointer;border:1px solid var(--border);background:transparent;transition:all 0.25s;font-family:inherit}
.chip:hover{border-color:var(--primary);color:var(--text)}
.chip.active{background:var(--primary);color:#fff;border-color:var(--primary);box-shadow:0 2px 16px var(--primary-dim)}
.mo{position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:200;display:none;align-items:center;justify-content:center;backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px)}
.mo.show{display:flex}
.mo-box{background:var(--surface2);border:1px solid var(--border2);border-radius:var(--radius);padding:28px;width:100%;max-width:500px;max-height:90vh;overflow-y:auto;box-shadow:var(--shadow);position:relative;animation:modalIn 0.3s ease}
@keyframes modalIn{from{opacity:0;transform:scale(0.95) translateY(20px)}to{opacity:1;transform:scale(1) translateY(0)}}
.mo-title{font-size:1.2rem;font-weight:800;color:var(--primary);margin-bottom:18px}
.mo-close{position:absolute;top:12px;right:16px;background:transparent;border:none;color:var(--text3);font-size:1.4rem;cursor:pointer;transition:all 0.2s}
.mo-close:hover{color:var(--text);transform:rotate(90deg)}
.toast{position:fixed;bottom:30px;left:50%;transform:translateX(-50%) translateY(20px);background:var(--surface);color:var(--text);border:1px solid var(--border2);border-radius:var(--radius-sm);padding:14px 28px;font-size:0.9rem;font-weight:600;opacity:0;transition:all 0.4s;z-index:999;backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);box-shadow:var(--shadow);pointer-events:none;text-align:center;max-width:90%}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.toast.err{border-color:var(--red)}
.sys-bar{height:6px;background:var(--border);border-radius:4px;overflow:hidden}
.sys-fill{height:100%;border-radius:4px;transition:width 0.6s ease}
.glass-btn-group{display:flex;flex-wrap:wrap;gap:4px;background:var(--surface3);padding:4px;border-radius:var(--radius-sm)}
.glass-btn{flex:1;min-width:70px;background:transparent;border:none;color:var(--text3);padding:8px 12px;border-radius:8px;cursor:pointer;font-weight:600;font-family:inherit;font-size:0.8rem;transition:all 0.3s}
.glass-btn.active{background:var(--primary);color:#fff;box-shadow:0 2px 16px var(--primary-dim)}
.glass-btn:hover:not(.active){background:var(--primary-dim);color:var(--text)}
.status-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(80px,1fr));gap:8px}
.status-card{padding:12px;border-radius:var(--radius-sm);text-align:center;cursor:pointer;font-weight:700;transition:all 0.3s;user-select:none;font-size:0.75rem;border:1px solid var(--border);background:var(--surface3)}
.status-card.active{border-color:var(--green);background:rgba(52,211,153,0.08);color:var(--green)}
.status-card.inactive{border-color:var(--border);color:var(--text3)}
.status-card .icon{font-size:1.4rem;display:block;margin-bottom:4px}
.mobile-nav{display:none;position:fixed;bottom:0;left:0;right:0;background:var(--surface);border-top:1px solid var(--border);z-index:99;backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);padding-bottom:env(safe-area-inset-bottom)}
.mobile-nav .items{display:flex;padding:6px 4px;justify-content:space-around}
.mobile-nav .item{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;padding:4px;color:var(--text3);font-size:0.6rem;cursor:pointer;transition:all 0.2s;border-radius:8px}
.mobile-nav .item.active{color:var(--primary);background:var(--primary-dim)}
.mobile-nav .item .icon{font-size:1.2rem}
.footer{height:var(--footer-h);display:flex;align-items:center;justify-content:center;font-size:0.75rem;color:var(--text3);border-top:1px solid var(--border);background:var(--surface);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);margin-top:auto}
.footer-inner{display:flex;align-items:center;gap:16px;flex-wrap:wrap;justify-content:center}
.footer-inner a{color:var(--primary-light);font-weight:600;transition:all 0.2s}
.footer-inner a:hover{text-shadow:0 0 20px var(--primary-dim)}
@media(max-width:768px){
  ...
  .glass-btn{min-width:60px;padding:6px 10px;font-size:0.7rem}
}
@media(max-width:480px){
  ...
  .mo-box{padding:20px;margin:12px}
}
.railway-hl{background:rgba(168,85,247,0.2)!important;color:#d8b4fe!important;border:1px solid #a855f7!important;font-weight:800}
.addr-scroll{max-height:280px;overflow-y:auto;border:1px solid var(--border);border-radius:var(--radius-sm);padding:6px}
.logs-scroll{max-height:350px;overflow-y:auto}
.scan-scroll{max-height:240px;overflow-y:auto}
.empty{padding:40px 20px;text-align:center;color:var(--text3);font-size:0.95rem}
</style>
</head>
<body>
<div class="toast" id="toast"></div>

<div id="login-page" style="display:none;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px">
  <div class="glass shadow-glow" style="max-width:400px;width:100%;padding:40px 32px;text-align:center">
    <div style="font-family:'Orbitron',sans-serif;font-size:2.2rem;font-weight:900;background:linear-gradient(135deg,var(--primary),var(--primary-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:4px">SulgX</div>
    <div style="font-size:0.75rem;color:var(--text3);margin-bottom:24px;letter-spacing:2px">PANEL v1.1.0</div>
    <div id="login-custom-message" style="margin-bottom:20px;color:var(--text2);font-size:0.9rem"></div>
    ...
    <button class="btn btn-primary" onclick="doLogin()" style="width:100%;justify-content:center;padding:14px;margin-top:8px">🔑 Login</button>
    <div id="login-err" style="color:var(--red);font-size:0.85rem;margin-top:10px;display:none">Invalid password</div>
...
<div id="dashboard-page" style="display:none">
  <header class="header">
    <div class="header-inner">
      <div style="display:flex;align-items:center;gap:12px">
        <span class="logo">SulgX <span>v1.1.0</span></span>
        <span id="panel-clock" style="font-weight:600;color:var(--primary);font-size:0.85rem"></span>
        <nav class="header-nav" id="mainNav">
          <button class="nav-link active" data-page="dashboard">📊 <span data-en="Dashboard">Dashboard</span></button>
          <button class="nav-link" data-page="inbounds">📡 <span data-en="Inbounds">Inbounds</span></button>
          <button class="nav-link" data-page="addresses">🔗 <span data-en="Clean IP">Clean IP</span></button>
          <button class="nav-link" data-page="ipscanner">🔍 <span data-en="Scanner">Scanner</span></button>
          <button class="nav-link" data-page="logs">📋 <span data-en="Logs">Logs</span></button>
          <button class="nav-link" data-page="telegram">🤖 <span data-en="Telegram">Telegram</span></button>
          <button class="nav-link" data-page="settings">⚙️ <span data-en="Settings">Settings</span></button>
        </nav>
      </div>
      <div class="header-right">
        <button class="btn btn-outline btn-sm" onclick="randomInbound()" data-en="+ Random">+ Random</button>
        ...
        <button class="btn btn-danger btn-sm" onclick="doLogout()" data-en="Logout">Logout</button>
  ...
  <main class="main">
    ...
    <section class="page active" id="page-dashboard">
      <div class="page-header">
        <div><div class="page-title">📊 Dashboard</div><div class="page-sub" id="last-up">—</div></div>
      </div>
      <div class="stats-grid">
        <div class="stat-card"><span class="stat-icon">📦</span><div class="stat-label">Traffic</div><div class="stat-val" id="sv-traffic">—<span class="stat-unit">MB</span></div></div>
        <div class="stat-card"><span class="stat-icon">📨</span><div class="stat-label">Requests</div><div class="stat-val" id="sv-requests">—</div></div>
        <div class="stat-card"><span class="stat-icon">⏱️</span><div class="stat-label">Uptime</div><div class="stat-val" id="sv-uptime" style="font-size:1.2rem">—</div></div>
        <div class="stat-card"><span class="stat-icon">💾</span><div class="stat-label">Disk Free</div><div class="stat-val" id="sv-disk">—<span class="stat-unit">GB</span></div></div>
      </div>
      <div class="stats-grid">
        <div class="stat-card"><span class="stat-icon">📥</span><div class="stat-label">Download</div><div class="stat-val" id="sv-down-speed" style="font-size:1.1rem">—<span class="stat-unit">KB/s</span></div></div>
        <div class="stat-card"><span class="stat-icon">📤</span><div class="stat-label">Upload</div><div class="stat-val" id="sv-up-speed" style="font-size:1.1rem">—<span class="stat-unit">KB/s</span></div></div>
        <div class="stat-card"><span class="stat-icon">📆</span><div class="stat-label">Monthly Usage</div><div class="stat-val" id="sv-monthly" style="font-size:1.1rem">—<span class="stat-unit">GB</span></div></div>
        <div class="stat-card"><span class="stat-icon">⚡</span><div class="stat-label">Status</div><div class="status-grid" id="settings-status" style="margin-top:4px"></div></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        <div class="card"><div class="card-header"><span class="card-title">💻 CPU</span><span id="cpu-v" style="font-weight:700;color:var(--primary)">—%</span></div><div class="sys-bar"><div class="sys-fill" id="cpu-b" style="background:var(--primary);width:0%"></div></div></div>
        <div class="card"><div class="card-header"><span class="card-title">🧠 Memory</span><span id="mem-v" style="font-weight:700;color:var(--green)">—%</span></div><div class="sys-bar"><div class="sys-fill" id="mem-b" style="background:var(--green);width:0%"></div></div></div>
      </div>
      <div class="card"><div class="card-header"><span class="card-title">📊 Hourly Traffic</span></div><div class="chart-wrap"><canvas id="tc"></canvas></div></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        <div class="card"><div class="card-header"><span class="card-title">🍩 Usage Distribution</span></div><div class="chart-wrap"><canvas id="doughnut-chart"></canvas></div></div>
        <div class="card"><div class="card-header"><span class="card-title">📈 Live Speed</span></div><div class="chart-wrap"><canvas id="speed-chart"></canvas></div></div>
      </div>
      <div class="card"><div class="card-header"><span class="card-title">🕒 Recent Activity</span></div><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Time</th><th>IP / Agent</th><th>Status</th></tr></thead><tbody id="login-logs-tbody"></tbody></table></div></div>
    </section>

    <!-- INBOUNDS -->
    <section class="page" id="page-inbounds">
      <div class="page-header">
        <div><div class="page-title">📡 Inbounds</div><div class="page-sub">Manage VLESS Configs</div></div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          <button class="btn btn-primary" onclick="showAddMo()">➕ Create</button>
          <button class="btn btn-outline btn-sm" onclick="exportLinks()">📤 Export</button>
          <button class="btn btn-outline btn-sm" onclick="document.getElementById('import-file').click()">📥 Import</button>
          <input type="file" id="import-file" style="display:none" accept=".json" onchange="importLinks(this)">
        </div>
      </div>
      <div style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap">
        <input id="srch" placeholder="Search..." oninput="filterLinks()" class="fi" style="flex:1;min-width:120px">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px">
        <button class="btn btn-outline btn-sm" onclick="batchAction('activate')">✅ Activate</button>
        <button class="btn btn-outline btn-sm" onclick="batchAction('deactivate')">⛔ Deactivate</button>
        <button class="btn btn-outline btn-sm" onclick="batchAction('reset_usage')">🔄 Reset Usage</button>
        <button class="btn btn-danger btn-sm" onclick="batchAction('delete')">🗑️ Delete</button>
      </div>
      <div class="card" style="padding:0;overflow:hidden">
        <div class="tbl-wrap"><table class="tbl" id="inbound-table"><thead><tr><th><input type="checkbox" id="select-all" onchange="toggleSelectAll()"></th><th onclick="sortLinks('label')">Name ↕</th><th>Type</th><th onclick="sortLinks('used_bytes')">Usage ↕</th><th>Conns</th><th onclick="sortLinks('expires_at')">Expiry ↕</th><th>Status</th><th>Actions</th></tr></thead><tbody id="ltb"></tbody></table></div>
    ...
    <section class="page" id="page-addresses">
      <div class="page-header"><div class="page-title">🔗 Clean IP</div></div>
      <div class="card">
        <div class="fg"><label class="fl">Add Addresses (one per line)</label><textarea class="fi" id="batch-addrs" rows="4" placeholder="8.8.8.8&#10;example.com"></textarea></div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          <button class="btn btn-primary" onclick="addBatchAddrs()">➕ Add All</button>
          <button class="btn btn-danger btn-sm" onclick="deleteAllAddrs()">🗑️ Delete All</button>
          <button class="btn btn-danger btn-sm" onclick="bulkDeleteAddrs()">🗑️ Delete Selected</button>
        </div>
    ...
    <section class="page" id="page-ipscanner">
      <div class="page-header"><div class="page-title">🔍 IP Scanner</div></div>
      <div style="background:rgba(251,191,36,0.08);border:1px solid rgba(251,191,36,0.2);border-radius:var(--radius-sm);padding:12px 16px;margin-bottom:14px;font-size:0.8rem;color:var(--yellow)">
        ⚠️ <strong>Safe Scan:</strong> Limited to 256 IPs at a time. Scanning is throttled to protect your hosting account.
        <div id="railway-note" style="display:none;margin-top:6px;color:#d8b4fe">ℹ️ For Railway, only Railway-related IPs will work.</div>
      </div>
      <div class="card">
        <div class="fg"><label class="fl">Provider</label><div id="provider-btns" class="chip-group"></div></div>
        <div class="fg" id="range-section" style="display:none"><label class="fl">Ranges</label><div id="range-btns" class="chip-group"></div></div>
        <div class="fg"><label class="fl">IPs / Domains / CIDR (one per line)</label><textarea class="fi" id="scan-ips" rows="5" placeholder="8.8.8.8&#10;example.com&#10;192.168.1.0/24"></textarea></div>
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          <button class="btn btn-primary" id="scan-start-btn" onclick="startIPScan()">🔍 Scan (port 443)</button>
          <button class="btn btn-danger btn-sm" id="scan-stop-btn" onclick="stopScan()" style="display:none">⏹️ Stop</button>
        </div>
        ...
        <div class="scan-scroll"><table class="tbl"><thead><tr><th>Address</th><th>Status</th><th>Latency</th></tr></thead><tbody id="scan-tbody"></tbody></table></div>
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">
          <button class="btn btn-outline btn-sm" onclick="sortBestIPs()">⭐ Sort Best</button>
          <button class="btn btn-outline btn-sm" onclick="copyReachableSorted()">📋 Copy Reachable</button>
        </div>
      </div>
    </section>
    ...
    <section class="page" id="page-logs">
      ...
      <div class="card" style="padding:0;overflow:hidden">
        <div class="logs-scroll"><table class="tbl"><thead><tr><th>#</th><th>Time (UTC)</th><th>Type</th><th>Event</th></tr></thead><tbody id="logs-tbody"></tbody></table></div>
        <div class="empty" id="logs-empty" style="display:none">No events recorded</div>
      </div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:8px">
    ...
    <section class="page" id="page-telegram">
      <div class="page-header"><div class="page-title">🤖 Telegram Bot</div></div>
      <div class="card">
        ...
        <div class="fg"><label class="fl">Notify Events</label><div style="display:flex;flex-wrap:wrap;gap:8px">
        ...
        <div class="fg"><label class="fl">Report Interval (hours)</label><input class="fi" type="number" id="tg-interval" value="1" min="0.5" step="0.5"></div>
        <div class="fg"><label class="fl">Telegram Language</label><div style="display:flex;align-items:center;gap:10px"><div class="toggle on" id="tg-lang-toggle" onpointerdown="toggleTgLang()"></div><span id="tg-lang-label">English</span><input type="hidden" id="tg-lang-hidden" value="en"></div></div>
        <div class="fg"><label class="fl">Custom Templates (EN)</label><textarea class="fi" id="tg-templates-en" rows="4">{"quota_90":"⚠️ {label} ({uid}) used 90% of quota","login":"🔐 SulgX Panel login\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {time}","expiry":"⏰ {label} expired","error":"❌ Error on {label}: check logs"}</textarea></div>
        <div class="fg"><label class="fl">Custom Templates (FA)</label><textarea class="fi" id="tg-templates-fa" rows="4">{"quota_90":"⚠️ {label} ({uid}) ۹۰٪ کوتا","login":"🔐 ورود SulgX\n🌐 IP: {ip}\n🤖 UA: {ua}\n📅 {time}","expiry":"⏰ {label} منقضی شد","error":"❌ خطا در {label}: بررسی شود"}</textarea></div>
    ...
    <section class="page" id="page-settings">
      ...
      <div class="card">
        <div class="fg"><label class="fl">Login Text</label><input class="fi" id="set-footer" placeholder="Custom login message"></div>
        ...
        <div class="fg"><label class="fl">Timezone</label>
          <div class="glass-btn-group" id="tz-glass-group">
            ...
            <button class="glass-btn" id="btn-tz-custom" onclick="toggleCustomTZInput(true)">Custom</button>
          </div>
        ...
        <div class="fg"><label class="fl">Theme</label>
          <div class="glass-btn-group" id="theme-glass-group">
            <button class="glass-btn active" id="btn-theme-dark" onclick="setPanelTheme('dark')">🌙 Dark</button>
            <button class="glass-btn" id="btn-theme-light" onclick="setPanelTheme('light')">☀️ Light</button>
            <button class="glass-btn" id="btn-theme-blue-dark" onclick="setPanelTheme('blue-dark')">🔵 Blue</button>
          </div>
          <input type="hidden" id="set-theme-color" value="dark">
        </div>

        <div class="fg"><label class="fl">Keep Alive</label>
          <div class="glass-btn-group" id="keepalive-mode-group">
            <button class="glass-btn active" id="btn-keepalive-simple" onclick="setKeepAliveMode('simple')">Simple</button>
            <button class="glass-btn" id="btn-keepalive-advanced" onclick="setKeepAliveMode('advanced')">Advanced</button>
          </div>
          <input type="hidden" id="set-keepalive-mode" value="simple">
          <div class="status-grid" style="margin-top:8px">
            <div class="status-card active" id="card-keepalive" onclick="toggleSettingCard('card-keepalive','set-keepalive-enabled')"><span class="icon">⚡</span>Keep-Alive<input type="hidden" id="set-keepalive-enabled" value="1"></div>
          </div>
          <div class="fg"><label class="fl">Interval (seconds)</label><input class="fi" type="number" id="set-keep-alive-interval" placeholder="300" min="60"></div>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
          <div class="fg"><label class="fl">Default Limit (GB)</label><input class="fi" type="number" id="set-default-limit" placeholder="0 = Unlimited"></div>
          ...
          <div class="fg"><label class="fl">Default Max Conns</label><input class="fi" type="number" id="set-default-maxconn" placeholder="0 = Unlimited"></div>
          <div class="fg"><label class="fl">Scanner Timeout (s)</label><input class="fi" type="number" id="set-scanner-timeout" placeholder="4"></div>
          <div class="fg"><label class="fl">Max Scan IPs</label><input class="fi" type="number" id="set-max-scan-ips" placeholder="256"></div>
          <div class="fg"><label class="fl">Monthly Limit (GB)</label><input class="fi" type="number" id="set-monthly-limit" placeholder="0 = Unlimited"></div>
        </div>

        <div class="fg"><label class="fl">System Toggles</label>
          <div class="status-grid">
            <div class="status-card active" id="card-log" onclick="toggleSettingCard('card-log','set-log-toggle')"><span class="icon">📝</span>Logs<input type="hidden" id="set-log-toggle" value="1"></div>
            <div class="status-card active" id="card-auto" onclick="toggleSettingCard('card-auto','set-auto-disable')"><span class="icon">🚫</span>Auto Disable<input type="hidden" id="set-auto-disable" value="1"></div>
            <div class="status-card active" id="card-tgrep" onclick="toggleSettingCard('card-tgrep','set-tg-report')"><span class="icon">📊</span>TG Reports<input type="hidden" id="set-tg-report" value="1"></div>
            <div class="status-card active" id="card-tgnot" onclick="toggleSettingCard('card-tgnot','set-tg-notify')"><span class="icon">🔔</span>TG Alerts<input type="hidden" id="set-tg-notify" value="1"></div>
          </div>
        </div>

        <hr style="border-color:var(--border);margin:16px 0">
        <div class="mo-title" style="margin-bottom:12px">🔐 Change Password</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div class="fg"><label class="fl">Current Password</label><input class="fi" type="password" id="cpw"></div>
          <div class="fg"><label class="fl">New Password</label><input class="fi" type="password" id="npw"></div>
        </div>
        <button class="btn btn-primary btn-sm" onclick="chgPw()">🔑 Update Password</button>
        <div style="margin-top:16px"><button class="btn btn-primary" onclick="saveGeneralSettings()" style="width:100%;justify-content:center;padding:12px">💾 Save All Settings</button></div>
        <hr style="border-color:var(--border);margin:16px 0">
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap">
          <button class="btn btn-danger" onclick="resetAllSettings()">🔄 Reset to Defaults</button>
          <span style="font-size:0.75rem;color:var(--text3)">Resets all settings except password.</span>
        </div>
      </div>
    </section>
  </main>
  ...
  <nav class="mobile-nav">
    <div class="items">
      <div class="item active" data-page="dashboard" onclick="switchPage('dashboard')"><span class="icon">📊</span>Home</div>
      <div class="item" data-page="inbounds" onclick="switchPage('inbounds')"><span class="icon">📡</span>Inbound</div>
      <div class="item" data-page="addresses" onclick="switchPage('addresses')"><span class="icon">🔗</span>Clean IP</div>
      <div class="item" data-page="ipscanner" onclick="switchPage('ipscanner')"><span class="icon">🔍</span>Scan</div>
      <div class="item" data-page="logs" onclick="switchPage('logs')"><span class="icon">📋</span>Logs</div>
      <div class="item" data-page="telegram" onclick="switchPage('telegram')"><span class="icon">🤖</span>Bot</div>
      <div class="item" data-page="settings" onclick="switchPage('settings')"><span class="icon">⚙️</span>Settings</div>
    </div>
  </nav>
...
<div class="mo-title">➕ Create Inbound</div>
<div class="fg"><label class="fl">Name</label><input class="fi" id="nl" placeholder="This Server is Free" maxlength="60"></div>
<div class="fg"><label class="fl">Flag / Country</label>
...
<div class="fg"><label class="fl">UUID</label><div style="display:flex;gap:6px"><input class="fi" id="auuid" placeholder="Auto-generate" style="flex:1"><button class="btn btn-outline btn-sm" onclick="generateUUID('auuid')">🎲 Generate</button></div></div>
<div class="fg"><button class="btn btn-outline btn-sm" onclick="toggleAdv('adv-create')" style="width:100%;justify-content:center">▼ Advanced Options</button>
  <div id="adv-create" class="adv-section" style="display:none;margin-top:8px">
    <div class="fg"><label class="fl">Profile</label><select class="fs" id="ares-profile" onchange="applyProfileCreate()"><option value="">Custom</option><option value="default">Default</option><option value="youtube">YouTube</option><option value="instagram">Instagram</option><option value="twitter">Twitter</option><option value="tiktok">TikTok</option><option value="whatsapp">WhatsApp</option><option value="telegram">Telegram</option><option value="netflix">Netflix</option><option value="spotify">Spotify</option><option value="google">Google</option></select></div>
...
<div class="fg"><label class="fl">Traffic Limit (GB)</label><input class="fi" type="number" id="nv" min="0" step="0.1" value="0" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Max Connections</label><input class="fi" type="number" id="nc" min="0" value="0" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Validity (Days)</label><input class="fi" type="number" id="nd" min="0" value="0" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Color</label><input type="color" id="alink-color" value="#7c3aed"></div>
<div style="display:flex;gap:6px;margin-top:12px"><button class="btn btn-primary" onclick="createLink()" style="flex:1">✅ Create</button><button class="btn btn-outline" onclick="document.getElementById('mo-add').classList.remove('show')">Cancel</button></div>
</div></div>
...
<div class="mo-title" id="et">✏️ Edit Inbound</div><input type="hidden" id="eu">
<div class="fg"><label class="fl">UUID</label><input class="fi" id="euuid" readonly></div>
<div class="fg"><label class="fl">Name</label><input class="fi" id="en2" maxlength="60"></div>
<div class="fg"><label class="fl">Flag / Country</label>
...
<div class="fg"><button class="btn btn-outline btn-sm" onclick="toggleAdv('adv-edit')" style="width:100%;justify-content:center">▼ Advanced Options</button>
  <div id="adv-edit" class="adv-section" style="display:none;margin-top:8px">
    <div class="fg"><label class="fl">Profile</label><select class="fs" id="eres-profile" onchange="applyProfile()"><option value="">Custom</option><option value="default">Default</option><option value="youtube">YouTube</option><option value="instagram">Instagram</option><option value="twitter">Twitter</option><option value="tiktok">TikTok</option><option value="whatsapp">WhatsApp</option><option value="telegram">Telegram</option><option value="netflix">Netflix</option><option value="spotify">Spotify</option><option value="google">Google</option></select></div>
...
<div class="fg"><label class="fl">Traffic Limit (GB)</label><input class="fi" type="number" id="el" min="0" step="0.1" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Max Connections</label><input class="fi" type="number" id="ec" min="0" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Validity (Days)</label><input class="fi" type="number" id="ed" min="0" placeholder="0 = Unlimited"></div>
<div class="fg"><label class="fl">Color</label><input type="color" id="e-color" value="#7c3aed"></div>
<div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:12px">
  <button class="btn btn-primary" onclick="saveEdit()" style="flex:1">💾 Save</button>
  <button class="btn btn-outline btn-sm" onclick="resetTraf()">🔄 Reset Traffic</button>
  <button class="btn btn-outline" onclick="document.getElementById('mo-edit').classList.remove('show')">Cancel</button>
</div>
</div></div>

<div class="mo" id="mo-qr"><div class="mo-box" style="max-width:360px"><button class="mo-close" onclick="document.getElementById('mo-qr').classList.remove('show')">✕</button>
<div class="mo-title">📷 QR Code</div><div class="qr-box" style="text-align:center;padding:16px;background:var(--surface3);border-radius:var(--radius-sm);border:1px solid var(--border)"><img id="qr-img" src="" alt="QR" style="max-width:200px;border-radius:8px"></div>
<button class="btn btn-primary" onclick="dlQR()" style="width:100%;justify-content:center;margin-top:10px">⬇️ Download</button>
</div></div>
...
<div class="mo-title">✏️ Edit Address</div>
<div class="fg"><label class="fl">New Address</label><input class="fi" id="edit-addr-input"></div>
<button class="btn btn-primary" onclick="saveAddrEdit()" style="width:100%;justify-content:center;margin-top:8px">💾 Save</button>
</div></div>

<script>
// ===== FULL SCRIPT (same logic, UI updated) =====
...
function setLang(l){lang=l;document.querySelectorAll('.lang-en,.lang-fa').forEach(e=>e.classList.remove('active'));document.querySelectorAll(`.lang-${l}`).forEach(e=>e.classList.add('active'));document.body.dir=l==='fa'?'rtl':'ltr';document.querySelectorAll('[data-en]').forEach(el=>{const v=el.getAttribute('data-'+l);if(v)el.textContent=v;});localStorage.setItem('ll',l);const footer=$m('footer-dedication');if(footer)footer.innerHTML=footerTexts[l]||footerTexts['en'];if(isAuthenticated){loadLoginLogs();loadLogs();renderAddrs();filterLinks();}}
function setTheme(t){theme=t;document.body.classList.toggle('light-mode',t==='light');document.body.classList.toggle('blue-mode',t==='blue-dark');localStorage.setItem('theme',t);document.querySelector('.btn-icon').textContent=t==='light'?'☀️':(t==='blue-dark'?'🌌':'🌙');if(tChart)updChartColors();syncGlassThemeButtons();}
...
function switchPage(id){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));$m('page-'+id).classList.add('active');document.querySelectorAll('.nav-link').forEach(n=>n.classList.toggle('active',n.dataset.page===id));document.querySelectorAll('.mobile-nav .item').forEach(n=>n.classList.toggle('active',n.dataset.page===id));}
